...
// Sky light uses raw light_block opacity (air/glass keep full sky light).
inline uint8_t MCPESkyLightAttenuation(uint8_t blockId) {
  return g_blockProps[blockId].light_block;
}

// Block light (torch/lava) attenuates at least by 1 per step.
inline uint8_t MCPEBlockLightAttenuation(uint8_t blockId) {
