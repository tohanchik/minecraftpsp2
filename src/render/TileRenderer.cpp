...
bool TileRenderer::tesselateCrossInWorld(uint8_t id, int lx, int ly, int lz, int cx, int cz) {
  ...
  float brightness = preferBlockLight(skyL, blkL) ? blkL : skyL;
...
static inline bool preferBlockLight(float skyL, float blkL) {
  // Small daylight bias: lets local lights stay visible under open sky near a source.
  return (blkL + 0.25f) > skyL;
}
...
bool TileRenderer::tesselateBlockInWorld(uint8_t id, int lx, int ly, int lz, int cx, int cz) {
  ...
  if (id == BLOCK_WATER_STILL || id == BLOCK_WATER_FLOW ||
    ...
    if (needFace(lx, ly, lz, cx, cz, id, 0, 1, 0, isFancy)) {
      ...
      float br = preferBlockLight(sl, bl) ? bl : sl;
  ...
  auto pickTess = [&](Tesselator *skyTess, Tesselator *fncTess,
    ...
    if (preferBlockLight(skyL, blkL)) return m_emitTess;
    return skyTess;
  };
  ...
  if (needFace(lx, ly, lz, cx, cz, id, 0, 1, 0, isFancy)) {
    ...
    bool useBlk = preferBlockLight(avgSky, avgBlk);
